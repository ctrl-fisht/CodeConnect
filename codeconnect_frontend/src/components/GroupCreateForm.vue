...
export default {
  ...
  methods: {
    handleAdd(formData) {
      ...
      this.$store.dispatch("user/createGroup", payload).then(
          data => {
    ...
    <Form name="add" @submit="handleAdd" :validation-schema="schema">
      <div v-if="!successful">
        <div class="form-group ml-2 flex flex-col">
          ...
          <Field class="h-8 pl-2  bg-white shadow" name="name" type="text"/>
          <ErrorMessage name="name" class="text-red-500"/>
        </div>
        <div class="form-group  ml-2 mt-4 flex flex-col">
          <label for="description" class="text-sm">Описание</label>
          <Field as="textarea" class="h-36 pl-2  bg-white shadow" name="description" type="text"/>
